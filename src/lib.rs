...
//! on PostgreSQL.
//!
//! The crate carries two of the four defense layers a public SQL surface
//! needs, and documents the other two:
//!
...
//!    helpers, including their double-quoted identifier forms.
//! 3. **Session battery** ([`session::build_setup_statements`]) — the
//!    `SET LOCAL` preamble that runs before every query: role switch, pinned
//!    `search_path`, timeouts, memory clamps, and
//!    `default_transaction_read_only = on` as an independent write-protection
//!    layer under the AST gate.
